...
let EventService = class EventService {
    ...
    async getAllEvents(obj) {
        const { query, paginationObj } = await this.apiService.getAllDocs(this.eventModel.find(), obj);
    ...
    async getAllPreviousReservedEvents(obj, user) {
        ...
        const { query, paginationObj } = await this.apiService.getAllDocs(this.eventModel.find(), obj, { _id: { $in: ids }, endedAt: { $lt: new Date() } });
        const events = await query
    ...
    async getAllFutureEvents(obj, user) {
        const tickets = await this.ticketModel.find({ user });
        const ids = tickets.map(({ event }) => event.toString());
        const { query, paginationObj } = await this.apiService.getAllDocs(this.eventModel.find(), obj, { _id: { $in: ids }, endedAt: { $gt: new Date() } });
        const events = await query
            .populate({ path: 'user', select: 'name mobile icon' })
            .select('-comments -likes -saved');
        return { events, pagination: paginationObj };
